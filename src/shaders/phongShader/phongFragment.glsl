...
void main(void) {
  ...
  visibility = useShadowMap(uShadowMap, vec4(shadowCoord, 1.0), bias_SM, 0.);

  float filterRadiusUV = 10.0 / 16384.0;

  //visibility = PCF(uShadowMap, vec4(shadowCoord, 1.0), bias_PCF, filterRadiusUV);
  
  visibility = PCSS(uShadowMap, vec4(shadowCoord, 1.0),bias_PCSS);
