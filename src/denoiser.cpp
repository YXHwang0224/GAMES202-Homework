...
#pragma omp parallel for
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            int xy_id = frameInfo.m_id(x,y);
            if (xy_id == -1)
                continue;
            //由于每个物体均有MVP变换，而每个物体唯一不变的是它们的物体坐标（local），
            //而我们有的是该帧时该像素点对应的id，从而得出其世界坐标，以及其在本帧的世界坐标到物体坐标（反转矩阵得到），
            //那么我们可以由此得到其物体坐标，而后通过访问该物体id所对应的上一帧的物体坐标到世界坐标的矩阵
            //最后将世界坐标变为屏幕坐标，而后通过得到该屏幕坐标像素所对应的物体id，与当前帧id对比即可
            Matrix4x4 WorldToLocal = Inverse(frameInfo.m_matrix[xy_id]);
            Matrix4x4 preLocalToWorld = m_preFrameInfo.m_matrix[xy_id];

            Float3 world_position = frameInfo.m_position(x, y);
            Float3 local_position = 
                WorldToLocal(world_position, Float3::EType::Point);
            Float3 pre_world_position =
                preLocalToWorld(local_position, Float3::EType::Point);
            Float3 pre_screen_position =
